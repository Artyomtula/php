...
$now = time();
$birthday = mktime(0, 0, 0, 5, 15, 2000); // Например, 15 мая 2000 года
...
<body>
    ...
    */
    ...
    $fmt = new IntlDateFormatter(
        ...
        "d MMMM Y 'года', EEEE HH:mm:ss"
    );
    
    echo "<p>Сегодня " . $fmt->format($now) . "</p>";
    
    // Вычисление дней до дня рождения
    // Определяем следующий день рождения
    $currentYear = (int)date('Y');
    $nextBirthday = mktime(0, 0, 0, 5, 15, $currentYear);
    
    // Если день рождения уже прошёл в этом году, берём следующий год
    if ($nextBirthday < $now) {
        $nextBirthday = mktime(0, 0, 0, 5, 15, $currentYear + 1);
    }
